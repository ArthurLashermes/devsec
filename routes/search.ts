...
module.exports = function searchProducts () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query(
      'SELECT * FROM Products WHERE (name LIKE :search_criteria OR description LIKE :search_criteria) AND deletedAt IS NULL ORDER BY name',
      {
        replacements: { search_criteria: `%${criteria}%` }
      }
    )
      .then(([products]: any) => {
